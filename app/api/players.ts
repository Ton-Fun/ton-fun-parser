...
export default (router: Router, db: Db) => {
    ...
    type Sorting = { $sort: number }

    const getSorting = (ctx: ExtendableContext): Sorting => {
        const orderBy: string = ctx.query.orderBy ? ctx.query.orderBy.toString() : DEFAULT_ORDER_BY
        const sort: string = ctx.query.sort ? ctx.query.sort.toString() : DEFAULT_SORT
