...
export default async (logger: Logger, mongo: MongoClient) => {
    ...
    await parser({
        ...
        betsFromTransactions: (transactions: TonTransaction[], state: State): Bet[] => {
            const filtered: TonTransaction[] = transactions.filter((transaction: TonTransaction) => {
                const transactionFromEcho: boolean = transaction.inMessage !== null &&
                    transaction.inMessage?.source !== null &&
                ...
                const notEnoughMoneyToPayReward: boolean = transaction.outMessages &&
    ...
    type TransactionBody = { address: Address, coins: BN }
